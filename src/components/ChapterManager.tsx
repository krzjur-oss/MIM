...
import { Chapter, QuizQuestion, EDUCATION_LEVELS, SCHOOL_TYPES, GRADES, isChapterProtected } from '../types';
...
export default function ChapterManager({
  ...
  const handleStartEditChapter = (ch: Chapter) => {
    if (isChapterProtected(ch)) {
      notify('Regulamin oraz Licencja WLPE są integralną częścią programu i nie podlegają edycji ani usunięciu.', 'info');
      return;
    }
  ...
  const handleSaveCreatedChapter = () => {
    ...
    if (isChapterProtected(currentEditingChapter)) {
      notify('Regulamin oraz Licencja WLPE są chronione przed modyfikacją.', 'error');
      return;
    }
              ...
              {filteredChaptersList.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                  {filteredChaptersList.map((ch) => {
                    ...
                    const isProtected = isChapterProtected(ch);
                      ...
                      >
                        <div>
                          ...
                          <div className="flex flex-wrap items-center gap-1.5 mb-2">
                            {isProtected && (
                              <span className="px-2 py-0.5 rounded-md bg-emerald-100 dark:bg-emerald-950 text-emerald-800 dark:text-emerald-300 font-extrabold text-[10px] flex items-center gap-1 border border-emerald-300 dark:border-emerald-800">
                                <ShieldCheck className="w-3 h-3 text-emerald-600 dark:text-emerald-400" />
                                <span>Tylko do odczytu (WLPE)</span>
                              </span>
                            )}
                        ...
                        <div className="flex items-center justify-between pt-3 border-t border-slate-100 dark:border-slate-800/80 text-xs">
                          ...
                          <div className="flex items-center gap-1.5">
                            {isProtected ? (
                              <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-semibold text-[11px] border border-slate-200 dark:border-slate-700">
                                <Lock className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />
                                <span>Temat zablokowany</span>
                              </div>
                            ) : (
                              <>
                                {/* Primary Edit Button */}
